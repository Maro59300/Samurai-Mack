...
class Fighter extends Sprite {
  ...
  update() {
    ...
    // )
    // c.fillRect(
    //  this.attackBox2.position.x,
    //   this.attackBox2.position.y,
    //   this.attackBox2.width,
    //   this.attackBox2.height
    // )
  ...
  attackplayer() {
    const Attacksound = new Audio('audio/Attack1.wav')
    
    if ((player.lastKey === 'd' && keys.Space.pressed)){
      player.switchSprite('attack1')
      player.attackBox
      player.isAttacking = true
      Attacksound.play()
    } else if (player.lastKey === 'a' && keys.Space.pressed){
  ...
  attackenemy() { 
    ...
    if ((enemy.lastKey === 'ArrowLeft' && keys.ArrowDown.pressed)){
      enemy.switchSprite('attack1')
      enemy.isAttacking = true
      Attacksound.play()
    } else if (enemy.lastKey === 'ArrowRight' && keys.ArrowDown.pressed ){
  ...
  takeHit() {
    this.health -= 5
    //death sprite bug cause have to be could in animate so it overwrites the restart values
    //if (this.health < 0) {
    //  this.switchSprite('death')
    //} else 
    this.switchSprite('takeHit')
    // Get Hit Sound
    const hitsound = new Audio('audio/Gethit.wav')
    hitsound.play()
